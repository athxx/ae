...
use crate::{
    prisma::PrismaClient,
    service::{Mutation, Query, Subscription},
};
...
pub async fn build_schema() -> AppSchema {
    let client = PrismaClient::_builder().build().await.expect("Failed to create Prisma client");
    ...
    client._migrate_deploy().await.expect("Failed to deploy database schema");

    // For more information about schema data, see: https://async-graphql.github.io/async-graphql/en/context.html#schema-data
    Schema::build(Query::default(), Mutation::default(), EmptySubscription).data(client).finish()
}
